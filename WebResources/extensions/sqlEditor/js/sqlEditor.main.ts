...
namespace Sql4CdsApp.SqlEditor {
    ...
    // ── Multi-tab state (one tab = one query document) ──────────────────
    // A single Ace editor + single Tabulator are shared; per-tab content is
    // swapped in/out. Each tab owns its own Ace EditSession (so cursor, undo
    // history and scroll persist) plus a snapshot of its last results.
    interface QueryTab {
        id: number;
        title: string;
        session: any;            // AceAjax edit session
        columns: any[] | null;   // Tabulator column defs
        data: any[] | null;      // Tabulator row objects
        commandMsg: string | null;
        rowsInfoText: string;
        errorText: string | null;
        statusText: string;
        running: boolean;
        runGen: number;          // bumped to invalidate in-flight / cancelled runs
...
`;
    ...
    export function onLoad() {
        ...
        document.getElementById("exportXlsxBtn")!.addEventListener("click", () => {
            const tab = getActiveTab();
            if (!tab || !tab.data || tab.data.length === 0) return;
            exportPopup.classList.remove("open");
            exportToggleBtn.setAttribute("aria-expanded", "false");
        ...
        document.getElementById("exportClipboardBtn")!.addEventListener("click", () => {
            const tab = getActiveTab();
            if (!tab || !tab.data || tab.data.length === 0 || !tab.columns) return;
            exportPopup.classList.remove("open");
            exportToggleBtn.setAttribute("aria-expanded", "false");
    ...
    function createTab(initialSql: string): QueryTab {
        const id = ++tabSeq;
    ...
    function buildTabEl(tab: QueryTab): HTMLElement {
        ...
        el.addEventListener("mousedown", (e: MouseEvent) => {
            if (e.button === 1) { e.preventDefault(); closeTab(tab.id); }     // middle-click closes
    ...
    function setupMetaDivider() {
        ...
        window.addEventListener("resize", () => {
            ...
            if (panel.classList.contains("collapsed")) return;  // don't fight the rail width
    ...
    function buildGridModel(result): { columns: any[]; data: any[] } {
    ...
    async function run() {
        ...
        try {
            ...
            if (tab.runGen !== myGen) return;   // cancelled, closed, or superseded
            tab.running = false;
            tab.loadStart = null;
    ...
    async function getAttributes(logicalName: string): Promise<any[]> {
    ...
    function buildTabDelimited(tab: QueryTab): string {
    ...
    const keywordCompleter: AceAjax.Completer = {
        getCompletions(_editor, session, pos, _prefix, callback) {
            if (dotContext(session, pos)) { callback(null, []); return; }   // after a dot only columns apply
    ...
    const tableCompleter: AceAjax.Completer = {
        getCompletions(_editor, session, pos, _prefix, callback) {
            if (dotContext(session, pos)) { callback(null, []); return; }   // dot = column territory
    ...
    function setupAutocomplete() {
        (ace as any).require("ace/ext/language_tools");   // installs Autocomplete + startAutocomplete cmd
